"""Event-GRU v2 at FINAL geometry: train to 378, predict at 408 — for a twink measurement.

Same model/config as fx_event_gru2.py (K=96, 12 event features incl. channel-funnel
panels, h=192, 7 epochs, 3 seeds); level set to EL. Output: submissions/evgru2_final.csv.
"""
import sys
import time
...
from common import NU, panel, target, rmsle, OUT, CACHE

DEV = "cuda"
TR, EV = [378, 350, 322, 294], 408
K, EPOCHS, BS, LR = 96, 7, 1024, 1.2e-3
SEEDS = [0, 1, 2]
EL = 2.3284
...
def build_events(a):
    ...
    valid = prev >= 0
    dshift = np.roll(d0, 1, axis=1)
    vshift = np.roll(valid, 1, axis=1)
    dshift[:, 0] = -1
    vshift[:, 0] = False
...
class EventGRU(nn.Module):
    def __init__(self, f=12, h=192):
        super().__init__()
        self.inp = nn.Linear(f, h)
        self.gru = nn.GRU(h, h, num_layers=2, batch_first=True, dropout=0.1)
        self.body = nn.Sequential(nn.Linear(2 * h, 160), nn.BatchNorm1d(160), nn.SiLU())
        self.p = nn.Linear(160, 1)
        self.m = nn.Linear(160, 1)

    def forward(self, x):
        z = torch.nn.functional.silu(self.inp(x))
        o, _ = self.gru(z)
        pooled = torch.cat([o[:, -1], o.mean(1)], 1)
        z = self.body(pooled)
...
t0 = time.time()
EVT = {a: build_events(a) for a in TR + [EV]}
...
for s in SEEDS:
    ...
    with torch.no_grad():
        for i in range(0, NU, 8192):
            ...
            o = model(torch.from_numpy(EVT[EV][idx].astype(np.float32)).to(DEV))
            acc.append((torch.sigmoid(o[:, 0]) * torch.clamp(o[:, 1], min=0)).float().cpu())
    lp = torch.cat(acc).numpy()
    outs.append(lp)
    np.save(OUT / f"evgru2_final_s{s}.npy", lp.astype(np.float32))
...
with open("submissions/evgru2_final.csv", "w") as f:
    f.write("user_id,predict\n")
    for u, v in zip(uids, pred):
        f.write(f"{u},{v:.6f}\n")
print(f"evgru2_final.csv written, mean log1p {np.log1p(pred).mean():.4f}")

import os
os._exit(0)   # обход шумного teardown CUDA
